...
import { adminGetDashboardStats } from "@/app/data-layer/admin/admin-get-dashboard-stats";

export async function SectionCards() {
  const { data: stats } = await adminGetDashboardStats();
  return (
    <div className="*:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card grid grid-cols-1 gap-4  *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:shadow-xs @xl/main:grid-cols-2 @5xl/main:grid-cols-4">
      <Card className="@container/card gap-4">
        <CardHeader className="flex flex-row space-y-0 justify-between w-full items-center">
          <div>
