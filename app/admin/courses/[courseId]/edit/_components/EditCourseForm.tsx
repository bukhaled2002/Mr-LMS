...
export default function EditCourseForm({
  ...
  const form = useForm<courseSchemaType>({
    ...
    defaultValues: {
      title: course?.title || "",
      category:
        (course?.category as courseSchemaType["category"]) || "Development",
      description: course?.description || "",
      duration: course?.duration || "1",
      fileKey: course?.fileKey || "", // <-- valid initial value
  ...
  function onSubmit(values: courseSchemaType) {
    startTransition(async () => {
      try {
        const res = await editCourse(course?.id as string, values);
        console.log(res);
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <FormItem>
              ...
              <FormControl>
                <div className="flex gap-2">
                  ...
                  >
                    Generate Slug
                    <SparkleIcon className="ml-2" size={16} />
                  </Button>
                </div>
              </FormControl>
              <FormMessage />
        ...
        <Button disabled={isPending} type="submit">
          {isPending ? (
            <>
              <Loader2 className="animate-spin" /> Updating Course...
            </>
          ) : (
            "Update Course"
          )}
